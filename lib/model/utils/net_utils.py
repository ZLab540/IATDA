...
class MemoryBank():
    def __init__(self, dim=64, size=1024, top_k=100, eps=1e-6):
        self.dim = dim
        self.size = size
        self.top_k = top_k
        self.eps = eps
        self.mb = []
        self.size_now = 0
        # self.sim = nn.CosineSimilarity(dim=0, eps=self.eps)

    def write(self, item):
        self.mb.append(item)
        if self.size_now < self.size:
            self.size_now += 1
        else:
            del self.mb[0]

    def sim(self, src_feat, tgt_feat):
    ...
    def read(self, item):
        ...
        for i in range(len(self.mb)):
            s = self.sim(item, self.mb[i])
            sim_list.append(s)
        sim_list = torch.Tensor(sim_list)
        sim_weight_top, indices = sim_list.topk(self.top_k, dim=0, largest=True, sorted=True)
        feat_top = [self.mb[i] for i in indices]
        feat_top = torch.stack(feat_top, dim=0)
        sim_weight_top = torch.Tensor(sim_weight_top).unsqueeze(-1)
        # soft_weight = F.softmax(sim_weight_top, dim=0)
        # feat_top = feat_top.cpu() * soft_weight
        # tmp_feat = torch.sum(feat_top, dim=0)
        feat_top = feat_top.cpu() * sim_weight_top
        tmp_feat = torch.sum(feat_top, dim=0) / (torch.sum(sim_weight_top) + self.eps)
        return tmp_feat.cuda()

# if __name__ =="__main__":
#     mem = MemoryBank(4, 5, 2)
