...
class kaist_ir(imdb):
    def __init__(self, image_set, num_shot=None):
        imdb.__init__(self, 'kaist_ir_' + image_set)
        self.num_shot = num_shot
        self._image_set = image_set
        self._data_path = os.path.join('data', 'kaist_ir')
        ...
        # 获得当前时间
        unix_time = time.time()
        time_array = time.localtime(unix_time)  # 格式化时间戳为本地时间
        self.time_now = time.strftime("%Y-%m-%d_%H:%M:%S", time_array)
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'kaist_ir')
    ...
    def _do_python_eval(self, output_dir='output', epoch = 10):
        ...
        if not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        # results = open(os.path.join(output_dir, 'results_'+self.time_now+'.txt'), 'a')
    ...
    def _do_matlab_eval(self, output_dir='output', epoch = 10, suffix=''):
        print('-----------------------------------------------------')
        print('Computing results with the official MATLAB eval code.')
        print('-----------------------------------------------------')
        # with open(os.path.join(output_dir, 'results_'+self.time_now+'.txt'), 'a') as results:
        with open(os.path.join(output_dir, 'result_mr.txt'), 'a') as results:
            results.write('Epoch {}:\n'.format(str(epoch)))
        ...
        cmd += 'kaist_eval_full(\'{:s}\',\'{:s}\'); quit;"' \
            .format(os.path.join(output_dir, 'det'+suffix), '/home/hy/dataset/kaist_test_anno_txt')
        print(('Running:\n{}'.format(cmd)))
    ...
    def evaluate_detections(self, all_boxes, output_dir, epoch = 10):
        ...
        if self.config['matlab_eval']:
            ...
            self._do_matlab_eval(output_dir, epoch=epoch)
