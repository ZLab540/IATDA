...
if __name__ == '__main__':
  ...
  if args.dataset == "pascal_voc":
      ...
      args.set_cfgs = ['ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '20']
  elif args.dataset == "kaist_ir":
      args.imdb_name = "kaist_ir_train"
      args.imdbval_name = "kaist_ir_test"
      args.set_cfgs = ['ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '20']
  elif args.dataset == "flir":
      args.imdb_name = "flir_train"
      args.imdbval_name = "flir_val"
      args.set_cfgs = ['ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '20']
  elif args.dataset == "flir_ir":
      args.imdb_name = "flir_ir_train"
      args.imdbval_name = "flir_ir_val"
      args.set_cfgs = ['ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '20']
  ...
  for i in range(num_images):
      ...
      if vis:
          if i % 20 == 0:
              cv2.imwrite(f'/media/disk4/hy/GPA/vis_kaist/baseline/{i}.png', im2show)
              print(f"{i}.png")
  ...
  #     inst_num = 1000
  #     for i in range(4):
  #         tmp = random.sample(fasterRCNN.vis_feat_list[i], inst_num)
  #         TSNE_feat.append(np.stack(tmp, 0))
  #         tmp = np.ones((inst_num,)) * i
  #         TSNE_label.append(tmp)
  #     TSNE_feat = np.concatenate(TSNE_feat, 0)
  #     TSNE_label = np.stack(TSNE_label, 0).flatten()
  #     np.savez('T_SNE_baseline.npz', X=TSNE_feat, Y=TSNE_label)
